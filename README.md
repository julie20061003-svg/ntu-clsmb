...
<body class="bg-slate-100 text-slate-800 min-h-screen py-8 px-4">
      ...
      <div id="auth-container" class="flex items-center gap-3 bg-slate-50 p-2.5 rounded-xl border border-slate-200">
        <div id="user-info" class="hidden flex items-center gap-2">
          ...
          <div class="text-xs">
            ...
            <div class="text-emerald-600 font-medium flex items-center gap-1">
              <span class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></span> 雲端實時同步中
            </div>
          </div>
  ...
  <script>
    ...
    function init() {
      // 1. 優先載入 Local 快取，避免 F5 重整時畫面閃爍變空白
      const localData = localStorage.getItem('ntu_mlsb_v12_cache');
      if (localData) {
        try {
          ...
          if (user) {
            ...
            unsubscribeSnapshot = docRef.onSnapshot(doc => {
              if (doc.exists) {
                ...
                state.adoptedPath = cloudData.adoptedPath || '1';
                state.completedReqIds = cloudData.completedReqIds || [];
                state.semesterLogs = cloudData.semesterLogs || [];
                // 寫入本地快取
                localStorage.setItem('ntu_mlsb_v12_cache', JSON.stringify(state));
                renderAll();
              } else {
                saveData(); // 建立初始文件
              }
    ...
    function handleLogout() {
      if (confirm("確定要登出嗎？登出後將清除此裝置上的本機快取。")) {
        // 主動點擊登出才清空 LocalStorage
        localStorage.removeItem('ntu_mlsb_v12_cache');
        state = {
          adoptedPath: '1',
          completedReqIds: [],
          semesterLogs: [],
    ...
    function saveData() {
      ...
      if (currentUser && db) {
        db.collection('users').doc(currentUser.uid).set({
          adoptedPath: state.adoptedPath,
          completedReqIds: state.completedReqIds,
          semesterLogs: state.semesterLogs,
