...
<body class="bg-slate-100 text-slate-800 min-h-screen py-8 px-4">
  ...
  <script>
    ...
    function init() {
      const localData = localStorage.getItem('ntu_mlsb_v11_data');
      if (localData) {
      ...
      if (auth) {
        auth.onAuthStateChanged(user => {
          if (user) {
            ...
            // 開啟 Firestore 實時雙向監聽 (Real-time Snapshot)
            ...
            unsubscribeSnapshot = docRef.onSnapshot(doc => {
              if (doc.exists) {
                ...
                localStorage.setItem('ntu_mlsb_v11_data', JSON.stringify(state));
                renderAll();
              } else {
                saveData(); // 初次登入推送到雲端
              }
            }, err => {
              alert("⚠️ 雲端讀取受阻：請確認 Firestore 資料庫『安全性』規則已點擊發布！");
            });
