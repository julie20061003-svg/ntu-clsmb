...
<body class="bg-slate-100 text-slate-800 min-h-screen py-8 px-4">
  ...
  <script>
    ...
    function clearLocalDataAndState() {
      localStorage.clear();
      state = {
        adoptedPath: '1',
        completedReqIds: [],
        ...
        auth.onAuthStateChanged(user => {
          if (user) {
            ...
            unsubscribeSnapshot = docRef.onSnapshot(doc => {
              ...
              alert("⚠️ 雲端讀取受阻：請確認 Firestore 資料庫『安全性』規則已發布！");
            });

          } else {
            // 未登入/登出狀態：清空記憶體與本機所有快取
            currentUser = null;
            clearLocalDataAndState();
    ...
    function saveData() {
      if (!currentUser) {
        alert("💡 請先點擊右上角「Google 帳號同步」登入，勾選與新增的課程才會儲存至雲端喔！");
        renderAll();
        return;
      }
