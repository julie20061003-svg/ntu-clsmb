...
<body class="bg-slate-100 text-slate-800 min-h-screen py-8 px-4">
  ...
  <div id="export-modal" class="hidden fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
    <div class="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl">
      ...
      <div class="flex justify-center p-4 bg-slate-50 rounded-xl border border-slate-200">
        <!-- 改為 Canvas 畫布渲染 QR Code -->
        <canvas id="qrcode" class="mx-auto"></canvas>
      </div>
      <div class="space-y-1.5">
        <label class="block text-xs font-semibold text-slate-700">專屬同步代碼：</label>
        <textarea id="export-code-text" readonly class="w-full h-20 p-2.5 text-xs bg-slate-50 border rounded-xl font-mono text-slate-600 select-all focus:outline-none resize-none break-all"></textarea>
  ...
  <script>
    ...
    function encodeStateData(data) {
      const miniObj = {
        p: data.adoptedPath,
        c: data.completedReqIds,
        s: data.semesterLogs
      };
      return btoa(encodeURIComponent(JSON.stringify(miniObj)));
    }
    ...
    function decodeStateData(codeStr) {
      ...
      return {
        adoptedPath: raw.p || raw.adoptedPath || '1',
        completedReqIds: raw.c || raw.completedReqIds || [],
        semesterLogs: raw.s || raw.semesterLogs || []
      };
    }

    function init() {
      // 1. 讀取本地快取
      const localData = localStorage.getItem('ntu_mlsb_v14_data');
      if (localData) {
        try { state = JSON.parse(localData); } catch(e) {}
      }

      // 2. 檢測 URL Hash (手機掃描 QR 開啟時自動匯入)
      if (window.location.hash.startsWith('#data=')) {
        ...
        try {
          ...
          if (importedState && Array.isArray(importedState.completedReqIds)) {
            state.adoptedPath = importedState.adoptedPath;
            state.completedReqIds = importedState.completedReqIds;
            state.semesterLogs = importedState.semesterLogs;
            localStorage.setItem('ntu_mlsb_v14_data', JSON.stringify(state));
            
            window.history.replaceState(null, null, window.location.pathname);
            alert('🎉 已成功透過 QR Code 匯入修課紀錄！');
          }
        } catch(e) {
