...
<body class="bg-slate-100 text-slate-800 min-h-screen py-8 px-4">
  <div class="max-w-5xl mx-auto space-y-6">
    ...
    <header class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      ...
      <!-- Google 帳號控制區 -->
      <div id="auth-container" class="flex items-center gap-3 bg-slate-50 p-2.5 rounded-xl border border-slate-200">
        <div id="user-info" class="hidden flex items-center gap-2">
          <img id="user-photo" class="w-8 h-8 rounded-full border border-slate-300" src="" alt="avatar">
          <div class="text-xs">
            <div id="user-name" class="font-bold text-slate-800"></div>
            <div class="text-emerald-600 font-medium">☁️ 雲端同步中</div>
          </div>
          <button onclick="handleLogout()" class="ml-2 px-2.5 py-1 text-xs text-slate-600 bg-white border rounded-lg hover:bg-slate-100">登出</button>
        </div>

        <button id="btn-login" onclick="handleLogin()" class="flex items-center gap-2 px-4 py-2 bg-white border border-slate-300 rounded-xl shadow-sm hover:bg-slate-50 text-xs sm:text-sm font-medium text-slate-700 transition">
    ...
    <section class="space-y-4">
      <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-3 gap-2">
          ...
          <div class="text-right">
            ...
            <span id="stat-total-remaining" class="text-2xl font-bold text-rose-500">128</span>
        ...
        <div class="bg-slate-50 p-2.5 rounded-lg border border-slate-200 text-xs text-slate-600 flex flex-wrap items-center gap-1">
          ...
          <span class="bg-purple-100 text-purple-800 px-1.5 py-0.5 rounded font-bold">無*通識/共同超修 (<span id="calc-excess">0</span>)</span> +
          <span>自由選修 (<b id="calc-ele" class="text-blue-600">0</b>)</span> =
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-1">
          ...
          <div class="text-lg font-bold text-slate-800"><span id="cat-earned-req">0</span> / 99</div>
          <div class="text-xs text-slate-400">還差 <span id="cat-rem-req" class="text-rose-500 font-semibold">99</span> 學分</div>
        </div>
        <div class="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-1">
          <div class="text-xs font-bold text-slate-500">共同必修 + 通識 (基本採計)</div>
          <div class="text-lg font-bold text-slate-800"><span id="cat-earned-common">0</span> / 24</div>
          <div class="text-xs text-slate-400">還差 <span id="cat-rem-common" class="text-rose-500 font-semibold">24</span> 學分</div>
        </div>
        <div class="bg-purple-50/80 p-4 rounded-xl border border-purple-200 shadow-sm space-y-1">
          <div class="text-xs font-bold text-purple-900 flex items-center justify-between">
            <span>✨ 通識/共同超修 (無*)</span>
            <span class="text-[10px] bg-purple-200 text-purple-800 px-1 rounded">轉選修</span>
          </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <label id="card-path-1" class="relative p-4 rounded-xl border-2 cursor-pointer transition space-y-3 block bg-slate-50 border-slate-200 hover:border-blue-300">
          ...
          <div class="flex justify-between items-center text-xs border-y py-1.5 border-slate-200/60">
            ...
            <span id="domain-status-p1" class="font-semibold text-rose-500">❌ 未達 2 領域</span>
          </div>
          <div class="space-y-1">
          ...
          <div class="text-xs font-semibold text-purple-700 bg-purple-50 p-2 rounded flex justify-between items-center">
            <span>✨ 此修法轉選修之超修學分 (無*)：</span>
        ...
        <label id="card-path-2" class="relative p-4 rounded-xl border-2 cursor-pointer transition space-y-3 block bg-slate-50 border-slate-200 hover:border-blue-300">
          ...
          <div class="flex justify-between items-center text-xs border-y py-1.5 border-slate-200/60">
            ...
            <span id="domain-status-p2" class="font-semibold text-rose-500">❌ 未達 3 領域</span>
          </div>
          <div class="space-y-1">
            <div class="flex justify-between text-xs font-medium text-slate-600">
              <span>基本採計進度</span>
  ...
  <script>
    ...
    function init() {
      const localData = localStorage.getItem('ntu_mlsb_v8_data');
      if (localData) {
    ...
    function handleLogin() {
      if (!auth) {
        alert("提示：Firebase SDK 未成功初始化。請確認網路連線是否正常！");
        return;
      }
    ...
    function saveData() {
      localStorage.setItem('ntu_mlsb_v8_data', JSON.stringify(state));
      if (currentUser && db) {
    ...
    function renderAll() {
      ...
      document.getElementById('domain-status-p1').className = p1_domain_ok ? "font-semibold text-emerald-600" : "font-semibold text-rose-500";
      document.getElementById('domain-status-p1').textContent = p1_domain_ok ? `✅ 已達標 (${takenDesignatedDomains.length}/2)` : `❌ 未達標 (${takenDesignatedDomains.length}/2)`;
      ...
      document.getElementById('domain-status-p2').className = p2_domain_ok ? "font-semibold text-emerald-600" : "font-semibold text-rose-500";
      document.getElementById('domain-status-p2').textContent = p2_domain_ok ? `✅ 已達標 (${takenDesignatedDomains.length}/3)` : `❌ 未達標 (${takenDesignatedDomains.length}/3)`;
      ...
      document.getElementById('calc-excess').textContent = activeExcess;
      document.getElementById('calc-ele').textContent = raw.ele;
