  ...
  <div class="max-w-5xl mx-auto space-y-6">
    ...
    <header class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
      ...
      <div class="flex items-center gap-3 bg-slate-50 p-3 rounded-xl border border-slate-200 text-xs">
        <div id="sync-status" class="flex items-center gap-2 text-slate-600 font-medium">
          <span id="sync-dot" class="w-2.5 h-2.5 rounded-full bg-amber-400"></span>
          <span id="sync-text">連線至試算表中...</span>
        </div>
      </div>
    </header>
  ...
  <script>
    ...
    function updateSyncUI(status, text) {
      ...
      if (status === 'loading') {
        ...
        textEl.textContent = text || "雲端同步中...";
      } else if (status === 'success') {
        dot.className = "w-2.5 h-2.5 rounded-full bg-emerald-500";
        textEl.textContent = text || "已與雲端試算表同步";
      } else {
        dot.className = "w-2.5 h-2.5 rounded-full bg-rose-500";
        textEl.textContent = text || "離線存取中";
      }
    }

    // 試算表 API 串接：讀取與儲存
    async function fetchFromGoogleSheet() {
      if (!GOOGLE_SHEET_SCRIPT_URL || GOOGLE_SHEET_SCRIPT_URL.includes("你的部署ID")) {
        updateSyncUI('error', '請先填入 Apps Script URL');
        return;
      }
      updateSyncUI('loading', '正在載入雲端試算表數據...');
      try {
        ...
        if (text && text.trim().startsWith('{')) {
          ...
          localStorage.setItem('ntu_mlsb_sheet_cache', JSON.stringify(state));
          updateSyncUI('success', '已連線 Google 試算表');
          renderAll();
        } else {
          updateSyncUI('success', '試算表為空，已初始化');
        }
      } catch (err) {
        console.error(err);
        updateSyncUI('error', '連線失敗，使用離線快取');
      }
    }
    ...
    async function saveToGoogleSheet() {
      ...
      if (!GOOGLE_SHEET_SCRIPT_URL || GOOGLE_SHEET_SCRIPT_URL.includes("你的部署ID")) return;

      updateSyncUI('loading', '同步儲存至試算表...');
      try {
        ...
        updateSyncUI('success', '變更已自動同步至試算表');
      } catch (err) {
        console.error(err);
        updateSyncUI('error', '儲存失敗 (已於本機快取)');
      }
    }

    function init() {
      // 1. 先載入本地快取，避免開啟網頁時畫面白屏
      const localData = localStorage.getItem('ntu_mlsb_sheet_cache');
      if (localData) {
        try { state = JSON.parse(localData); } catch(e) {}
      }
      renderAll();

      // 2. 從 Google 試算表拉取最新數據
